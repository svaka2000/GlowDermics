...
const TABS = ['Signs', 'Causes', 'Protocol', 'Ingredients', 'Barrier Lipids'];
...
function buildProtocolSteps(Colors: ShimColors) {
  ...
  { step: 3, phase: 'Days 3–14: Rebuild lipids', action: 'Apply occlusive + ceramide combo', detail: 'The holy grail combo: ceramide-containing moisturiser (CeraVe, La Roche-Posay Cicaplast) or a lipid-rich balm as your lipid donor. Apply while skin is still slightly damp to trap water. Top with a thin occlusive layer at night.', warning: null, color: Colors.teal },
...
const REPAIR_INGREDIENTS = [
  { name: 'Ceramides', role: 'Primary lipid of stratum corneum (50% of barrier lipids). Direct structural rebuild.', found: 'CeraVe, lipid-rich balms (contain ceramide precursors), sunflower seed oil', use: 'AM + PM moisturiser', rating: 'essential' },
  { name: 'Cholesterol', role: 'Co-lipid (25% of barrier lipids). Works synergistically with ceramides. Must be balanced at correct ratio.', found: 'Animal-fat balms (high cholesterol content), egg yolk oil', use: 'PM', rating: 'essential' },
  { name: 'Fatty Acids (C18)', role: 'Third barrier lipid fraction (15%). Oleic + linoleic acid. Linoleic is anti-inflammatory and ceramide-building.', found: 'Olive/animal-fat occlusives (oleic), rosehip (linoleic), hemp seed (balanced)', use: 'PM', rating: 'essential' },
...
const OCCLUSIVE_POINTS = [
  ...
  { title: 'Anti-inflammatory fatty acids reduce repair-time inflammation', body: 'Palmitoleic acid (found in some pasture-raised animal fats) has documented antimicrobial and anti-inflammatory effects. Reduces the inflammatory cycle that compounds barrier damage.' },
  { title: 'Occlusive without being fully occlusive', body: 'Unlike petrolatum (100% occlusive), a lipid-rich balm allows some gas exchange while still dramatically reducing TEWL. It creates the ideal repair environment: moisture is retained, but CO2 and oxygen still exchange through the barrier.' },
  { title: 'Application timing matters', body: 'Apply the occlusive within 60 seconds of washing while skin is still slightly damp. Skin warmth melts a balm-type occlusive instantly — use a pea-sized amount. Thicker application for severe barrier damage, thinner for maintenance.' },
];
