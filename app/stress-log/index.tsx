...
function buildStressColors(c: Palette) {
  return ['', '#4ADE80', '#86EFAC', c.gold, '#FCA5A5', c.scorePoor];
}
const STRESS_EMOJIS = ['', '😌', '🙂', '😐', '😟', '😖'];
...
export default function StressLog() {
  ...
  const STRESS_COLORS = useMemo(() => buildStressColors(colors), [colors]);
...
function makeStyles(c: Palette) {
  ...
  todayQuestion: { fontSize: 16, fontWeight: '700', color: c.textPrimary },

  levelRow: { flexDirection: 'row', gap: 6 },
  levelBtn: { flex: 1, alignItems: 'center', gap: 3, paddingVertical: 10, borderRadius: 12, borderWidth: 1, borderColor: c.border, backgroundColor: c.bgElevated },
  levelEmoji: { fontSize: 18 },
  levelNum: { fontSize: 15, fontWeight: '800', color: c.textPrimary },
  levelLabel: { fontSize: 8, fontWeight: '600', color: c.textMuted, textAlign: 'center' },
  ...
  saveBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, height: 48, borderRadius: 12, backgroundColor: c.primary, overflow: 'hidden' },
  ...
  scienceTip: { flex: 1, fontSize: 13, color: c.textSecondary, lineHeight: 20 },
  ...
  logDate: { fontSize: 13, fontWeight: '600', color: c.textPrimary },
