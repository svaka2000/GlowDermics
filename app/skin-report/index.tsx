...
import {
  View, Text, StyleSheet, ScrollView, Pressable, Share, ActivityIndicator, Animated, Easing,
} from 'react-native';
...
Return this exact JSON:
{
  ...
  "journeyMoment": "1-2 brand-agnostic sentences with the single highest-impact insight for their specific skin situation — no products, no brand",
  ...
  const shareReport = async () => {
    if (!report) return;
    const text = `My Velumi AI Skin Report
    ...
    <View style={styles.root}>
      <SafeAreaView edges={['top']}>
        <Animated.View style={[styles.header, { opacity: headerAnim, transform: [{ translateY: headerAnim.interpolate({ inputRange: [0, 1], outputRange: [-14, 0] }) }] }]}>
          ...
          {report && (
            <Pressable accessibilityRole="button" accessibilityLabel="Share report" hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} style={styles.shareBtn} onPress={shareReport}>
        ...
        {report && (
          <>
            ...
            <View style={styles.scoreCard}>
              <LinearGradient colors={[colors.primary + '14', colors.primary + '05']} style={StyleSheet.absoluteFill} />
            ...
            <View style={styles.barrierCard}>
              ...
              <Text style={styles.barrierTitle}>✨ Your Skin Insight</Text>
              <Text style={styles.barrierText}>{report.journeyMoment}</Text>
            </View>
...
function makeStyles(c: Palette) {
  ...
  emptyTitle: { fontSize: 22, fontWeight: '900', color: c.white, fontFamily: fonts.display },
  emptyDesc: { fontSize: 14, color: 'rgba(255,255,255,0.75)', textAlign: 'center', lineHeight: 22 },
  generateBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    backgroundColor: c.textPrimary, paddingHorizontal: 24, paddingVertical: 14,
    borderRadius: 14, marginTop: 8,
  },
  generateBtnText: { fontSize: 16, fontWeight: '800', color: c.bg },
  ...
  errorText: { fontSize: 14, color: c.scorePoor, textAlign: 'center', marginBottom: 14 },

  gradeHero: {
    borderRadius: 20, overflow: 'hidden', borderWidth: 2,
    padding: 28, gap: 8, marginBottom: 14, alignItems: 'center',
  },
  gradeText: { fontSize: 72, fontWeight: '900', fontFamily: fonts.display },
  gradeLabelText: { fontSize: 16, fontWeight: '800', color: c.textSecondary },
  gradeHeadline: { fontSize: 14, color: c.textMuted, textAlign: 'center', fontStyle: 'italic', lineHeight: 20 },
  ...
  summaryText: { fontSize: 14, color: c.textSecondary, lineHeight: 22 },
  ...
  scoreLabel: { fontSize: 10, color: c.textMuted, fontWeight: '700', textTransform: 'uppercase' },
  trendRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  trendNote: { flex: 1, fontSize: 13, color: c.textMuted, lineHeight: 19 },
  ...
  insightNum: { fontSize: 11, fontWeight: '900', color: c.primary },
  ...
  goalText: { fontSize: 18, fontWeight: '800', color: c.white, lineHeight: 26 },
  ...
  closeText: { fontSize: 15, color: c.textSecondary, textAlign: 'center', lineHeight: 24, fontStyle: 'italic' },

  regenRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 },
  regenDate: { fontSize: 11, color: c.textMuted },
  regenBtn: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  regenText: { fontSize: 12, color: c.textMuted, textDecorationLine: 'underline' },
  });
}
